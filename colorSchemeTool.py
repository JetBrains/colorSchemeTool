...
typedef = Attribute("TYPEDEF", text, 'storage.type')
...
conditionally_not_compiled = Attribute("CONDITIONALLY_NOT_COMPILED", block_comment)
...
ivar = Attribute("IVAR", text, scope='variable')
oc_local_variable = Attribute("OC.LOCAL_VARIABLE", text, scope='variable')
...
oc_property = Attribute("OC.PROPERTY", ivar)
oc_keyword = Attribute("OC.KEYWORD", keyword)
label = Attribute("LABEL", oc_keyword)
oc_number = Attribute("OC.NUMBER", number)
oc_selfsuperthis = Attribute("OC.SELFSUPERTHIS", oc_keyword)
...
class_reference = Attribute("CLASS_REFERENCE", typedef, scope='entity.name.class')
protocol_reference = Attribute("PROTOCOL_REFERENCE", class_reference, scope='storage.type') 
message_argument = Attribute("MESSAGE_ARGUMENT", text, scope='entity.name.function')
oc_function = Attribute("OC.FUNCTION", message_argument)
oc_parameter = Attribute("OC.PARAMETER", text, scope='variable.parameter')
...
enum_const = Attribute("ENUM_CONST", text, scope='constant.other.symbol')
macroname = Attribute("MACRONAME", oc_function)
macro_parameter = Attribute("MACRO_PARAMETER", text)
...
def write_idea_scheme(filename):
    ...
    ET.SubElement(scheme, 'option', name='EDITOR_FONT_SIZE', value='13')
